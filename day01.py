...
# Replace L with '-', remove R
def prepareData(data):
    return str(data).replace('L', '-').replace('R', '').splitlines()

# Task - count when dial points 0
# dial size = 0 .. 99 => 100
# points 0 when sum % 100 = 0
def solution01(lines):
    total = 0
    current = start
...
# Test example data
assert(answer1 == solution01(example1))
assert(answer2 == solution02(example1))  
